...
$configFile = __DIR__ . '/_setting_shop_producers_categories.inc';
...
\$SITES = [
    'site.boostore.pro' => [
        ...
        'date_offset_base' => '',
        'date_offset_days' => 1,
        'export_producer_id' => false,
        'export_producer_id' => false,
        'export_producer_name' => true,
        'per_page' => 200,
...
$API_URL = 'https://' . $currentSite . '/api/commerce/products/producers';

// Extract per-site config into global variables (backward-compatible)
$siteCfg = $SITES[$currentSite] ?? [];
$ALLOWED_PRODUCERS  = $siteCfg['allowed_producers'] ?? ($ALLOWED_PRODUCERS ?? []);
...
$EXPORT_PRODUCER_ID   = $siteCfg['export_producer_id'] ?? ($EXPORT_PRODUCER_ID ?? false);
$EXPORT_PRODUCER_ID  = $siteCfg['export_producer_id'] ?? ($EXPORT_PRODUCER_ID ?? false);
$EXPORT_PRODUCER_NAME = $siteCfg['export_producer_name'] ?? ($EXPORT_PRODUCER_NAME ?? true);
...
$CATEGORIES_DIR = $SITE_DIR . DIRECTORY_SEPARATOR . 'shop_producers';
// Ensure site directory + blog subfolder exist
if (!is_dir($CATEGORIES_DIR)) { @mkdir($CATEGORIES_DIR, 0777, true); }
...
function sitesExport($sites) {
    ...
    foreach ($sites as $sDomain => $sCfg) {
        ...
        $producers = $sCfg['allowed_producers'] ?? [];
        if (!empty($producers)) {
            $c .= "        'allowed_producers' => [\n";
            foreach ($producers as $cid => $cname) {
        ...
        $c .= "        'export_producer_id' => ".($sCfg['export_producer_id'] ?? false ? 'true' : 'false').",\n";
        $c .= "        'export_producer_id' => ".($sCfg['export_producer_id'] ?? false ? 'true' : 'false').",\n";
        $c .= "        'export_producer_name' => ".($sCfg['export_producer_name'] ?? true ? 'true' : 'false').",\n";
...
// ---- Хлебные крошки ----
$crumbs = ['<a href="index.php" style="color:#00d4ff;text-decoration:none;font-weight:600;"><span data-i18n="home">🏠 Главная</span></a>', '<a href="?site='.urlencode($currentSite).'" style="color:#00d4ff;text-decoration:none;font-weight:600;">📝 <span data-i18n="entity_name">Производители</span></a>'];
...
<h1 style="margin:0 0 20px 0;">▸ <span data-i18n="title">Управление производителями магазина — Boostore.pro</span></h1>
...
// ===================================================================
// _get-shop_producers_categories.php — ЭКСПОРТ (получение производителей с API)
// ===================================================================
if ($action === 'get'):
@set_time_limit(300);
...
<p style="font-size:11px;color:#888;margin-bottom:10px;" data-i18n="prod_note">Если не выбрано ни одной — обрабатываются все производителя.</p>
<div id="get-prods"><?php $producerParentIdx=0;foreach($ALLOWED_PRODUCERS as $cid=>$cname):?>
<div class="prod-row"><input type="text" name="cat[<?=$producerParentIdx?>][id]" value="<?=$cid?>" placeholder="ID" data-i18n-placeholder="cat_id_placeholder" style="max-width:80px;"><input type="text" name="cat[<?=$producerParentIdx?>][name]" value="<?=htmlspecialchars($cname)?>" placeholder="имя производителя" data-i18n-placeholder="cat_name_placeholder"><button type="button" onclick="this.parentElement.remove()" class="btn-sm">✕</button></div>
<?php $producerParentIdx++;endforeach;?></div>
<button type="button" onclick="var d=document.getElementById('get-prods'),i=d.children.length;d.innerHTML+='<div class=\'prod-row\'><input type=\'text\' name=\'cat['+i+'][id]\' placeholder=\'ID\' data-i18n-placeholder=\'cat_id_placeholder\' style=\'max-width:80px;\'><input type=\'text\' name=\'cat['+i+'][name]\' placeholder=\'имя производителя\' data-i18n-placeholder=\'cat_name_placeholder\'><button type=\'button\' onclick=\'this.parentElement.remove()\' class=\'btn-sm\'>✕</button></div>';" style="padding:4px 12px;background:#0f3460;color:#00d4ff;border:1px solid #00d4ff;border-radius:4px;cursor:pointer;font-size:12px;margin-top:4px;" data-i18n="btn_add_cat">+ Добавить категорию</button>
<input type="hidden" name="prods_configured" value="1">
</div>
<div class="card">
  <h3 style="margin:0 0 10px;font-size:15px;color:#4dc9f6;" data-i18n="fix_import_title">🔧 Синхронизация полей производителя</h3>
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:8px;">
    <div>
      <label style="font-size:11px;color:#888;display:block;margin-bottom:3px;" data-i18n="ref_lang_label">🌐 Язык эталонной производителя</label>
...
var _t={ru:{'home':'🏠 Главная','site_label':'Сайт:','entity_name':'Производители','import':'📥 Импорт','export':'📤 Экспорт','completed':'✓ Результат','result_label':'✓ Результат','created_label':'✅ Создано:','updated_label':'📝 Обновлено:','errors_label':'❌ Ошибок:','skipped_exist_label':'⏭ Пропущено (существуют):','skipped_exist_label2':'⏭ Пропущено','export_text_only':'📝 Экспортировать только текст','file_selection':'📁 Выбор файлов','btn_get':'📥 СКАЧАТЬ','btn_update':'📤 ОТПРАВИТЬ','plaque_import':'▸ <strong>Настройки импорта</strong> — получение производителей с API','plaque_export':'▸ <strong>Настройки экспорта</strong> — отправка производителей на Boostore.pro','search_import':'Поиск по имени (slug)','prod_filter':'📂 Производители для фильтрации','prod_note':'Если не выбрано ни одной — обрабатываются все производителя.','back_home':'← Назад','btn_more':'+ ЕЩЕ','btn_more_multi':'📋 ЕЩЕ НЕСКОЛЬКО','btn_add_cat':'+ Добавить категорию','per_page_import':'Производителей за запрос','date_from':'Дата с','date_to':'Дата по','lang_label':'Язык','id_min_label':'ID >','id_max_label':'ID <','id_min_placeholder':'1000','id_max_placeholder':'5000','folder_planned_chk':'Разделять planned в <code>products/planned/</code>','folder_producer_chk':'Разделять по папкам производителей','all_languages':'все','lang_ru':'Русский','lang_en':'English','lang_ua':'Українська','lang_pl':'Polski','lang_de':'Deutsch','lang_fr':'Français','lang_es':'Español','lang_it':'Italiano','lang_kk':'Қазақ','lang_be':'Беларуская','api_docs':'API Docs','version':'v2.0','date_format':'ГГГГ-ММ-ДД','search_placeholder':'часть имени, например: shoes','cat_id_placeholder':'ID','cat_name_placeholder':'имя производителя','prompt_values':'Введите значения (каждая строка — отдельное поле):','step_forward':'➡ ДАЛЕЕ','dry_run_label':'Dry run','filter_name':'Фильтр по имени (slug)','batch_label':'Отправить за 1 раз','ref_lang_be':'Белорусский (be)','ref_lang_en':'English (en)','ref_lang_ru':'Русский (ru)','ref_lang_ua':'Українська (ua)','ref_lang_pl':'Polski (pl)','date_mode_meta':'Из мета-данных (дата из каждого производителя)','date_mode_fixed':'Одна дата для всех производителей','date_mode_offset':'Смещение дат (+N дней на производитель)','planned_notset':'— не указано (из мета-данных)','planned_0':'0 — не отложенная','planned_1':'1 — отложенная публикация','status_mode_meta':'Из мета-данных (статус из каждого производителя)','status_mode_override':'Переопределить для всех производителей','date_mode_label':'📅 Режим даты публикации','title':'Управление производителями магазина — Boostore.pro','fix_import_title':'🔧 Исправление по эталону','fix_import_desc':'Синхронизировать поля с эталонным производителем (по slug) после сохранения','ref_lang_label':'🌐 Язык эталонной производителя','export_fields_label':'📋 Поля для экспорта','export_producer_id_label':'ID виробника','export_product_id_label':'Product ID','export_producer_name_label':'Имя производителя','import_only_named':'Только с именем'},en:{'home':'🏠 Home','site_label':'Site:','entity_name':'Producers','import':'📥 Import','export':'📤 Export','completed':'✓ Completed','result_label':'✓ Result','created_label':'✅ Created:','updated_label':'📝 Updated:','errors_label':'❌ Errors:','skipped_exist_label':'⏭ Skipped (exist):','skipped_exist_label2':'⏭ Skipped','export_text_only':'📝 Export text only','file_selection':'📁 File selection','btn_get':'📥 DOWNLOAD','btn_update':'📤 UPLOAD','plaque_import':'▸ <strong>Import Settings</strong> — fetching products from API','plaque_export':'▸ <strong>Export Settings</strong> — sending products to Boostore.pro','search_import':'Search by name (slug)','prod_filter':'📂 Categories for filtering','prod_note':'If none selected, all categories are processed.','back_home':'← Back','btn_more':'+ MORE','btn_more_multi':'📋 ADD MULTIPLE','btn_add_cat':'+ Add Category','per_page_import':'Products per request','date_from':'Date from','date_to':'Date to','lang_label':'Language','folder_planned_chk':'Separate planned into <code>products/planned/</code>','folder_producer_chk':'Separate by category folders','all_languages':'all','lang_ru':'Russian','lang_en':'English','lang_ua':'Ukrainian','lang_pl':'Polish','lang_de':'German','lang_fr':'French','lang_es':'Spanish','lang_it':'Italian','lang_kk':'Kazakh','lang_be':'Belarusian','api_docs':'API Docs','version':'v2.0','date_format':'YYYY-MM-DD','search_placeholder':'part of name, e.g.: shoes','cat_id_placeholder':'ID','cat_name_placeholder':'category name','prompt_values':'Enter values (each line is a separate field):','step_forward':'➡ NEXT','dry_run_label':'Dry run','filter_name':'Filter by name (slug)','batch_label':'Send per run','ref_lang_be':'Belarusian (be)','ref_lang_en':'English (en)','ref_lang_ru':'Russian (ru)','ref_lang_ua':'Ukrainian (ua)','ref_lang_pl':'Polish (pl)','date_mode_meta':'From meta-data (date from each article)','date_mode_fixed':'Single date for all products','date_mode_offset':'Date offset (+N days per article)','planned_notset':'— not set (from meta-data)','planned_0':'0 — not planned','planned_1':'1 — planned publishing','status_mode_meta':'From meta-data (status from each article)','status_mode_override':'Override for all products','date_mode_label':'📅 Publication Date Mode','title':'Producer Management — Boostore.pro','fix_import_title':'🔧 Fix by Reference','fix_import_desc':'Sync fields with reference producer (by slug) after save','ref_lang_label':'🌐 Reference Language','export_fields_label':'📋 Fields to export','export_producer_id_label':'Producer ID','export_product_id_label':'Product ID','export_producer_name_label':'Producer name','import_only_named':'Named only'},ua:{'home':'🏠 Головна','site_label':'Сайт:','entity_name':'Виробники','import':'📥 Імпорт','export':'📤 Експорт','completed':'✓ Результат','result_label':'✓ Результат','created_label':'✅ Створено:','updated_label':'📝 Оновлено:','errors_label':'❌ Помилок:','skipped_exist_label':'⏭ Пропущено (існують):','skipped_exist_label2':'⏭ Пропущено','export_text_only':'📝 Експортувати тільки текст','file_selection':'📁 Вибір файлів','btn_get':'📥 ПОЧАТИ ІМПОРТ','btn_update':'📤 ПОЧАТИ ЕКСПОРТ','plaque_import':'▸ <strong>Налаштування імпорту</strong> — отримання производителей з API','plaque_export':'▸ <strong>Налаштування експорту</strong> — відправлення производителей на Boostore.pro','search_import':'Пошук за іменем (slug)','prod_filter':'📂 Категорії для фільтрації','prod_note':'Якщо не вибрано жодної — обробляються всі категорії.','back_home':'← На головну','btn_more':'+ ЩЕ','btn_more_multi':'📋 ДОДАТИ КІЛЬКА','btn_add_cat':'+ Додати категорію','per_page_import':'Статей за запит','date_from':'Дата з','date_to':'Дата по','lang_label':'Мова','folder_planned_chk':'Розділяти planned у <code>products/planned/</code>','folder_producer_chk':'Розділяти по папках категорій','all_languages':'всі','lang_ru':'Російська','lang_en':'Англійська','lang_ua':'Українська','lang_pl':'Польська','lang_de':'Німецька','lang_fr':'Французька','lang_es':'Іспанська','lang_it':'Італійська','lang_kk':'Казахська','lang_be':'Білоруська','api_docs':'API Docs','version':'v2.0','date_format':'РРРР-ММ-ДД','search_placeholder':'частина імені, наприклад: shoes','cat_id_placeholder':'ID','cat_name_placeholder':'ім\'я категорії','prompt_values':'Введіть значення (кожен рядок — окреме поле):','step_forward':'➡ ДАЛІ','dry_run_label':'Dry run','filter_name':'Фільтр за іменем (slug)','batch_label':'Відправити за 1 раз','ref_lang_be':'Білоруська (be)','ref_lang_en':'Англійська (en)','ref_lang_ru':'Російська (ru)','ref_lang_ua':'Українська (ua)','ref_lang_pl':'Польська (pl)','date_mode_meta':'З мета-даних (дата з кожної статті)','date_mode_fixed':'Одна дата для всіх производителей','date_mode_offset':'Зміщення дат (+N днів на статтю)','planned_notset':'— не вказано (з мета-даних)','planned_0':'0 — не відкладена','planned_1':'1 — відкладена публікація','status_mode_meta':'З мета-даних (статус з кожної статті)','status_mode_override':'Перевизначити для всіх производителей','date_mode_label':'📅 Режим дати публікації','title':'Керування виробниками магазину — Boostore.pro','fix_import_title':'🔧 Виправлення за еталоном','fix_import_desc':'Синхронізувати поля з еталонним виробником (по slug) після збереження','ref_lang_label':'🌐 Мова еталонного виробника','export_fields_label':'📋 Поля для експорту','export_producer_id_label':'ID виробника','export_product_id_label':'ID товару','export_producer_name_label':'Ім\'я виробника','import_only_named':'Тільки з назвою'}};
...
// Parse categories from form if submitted (prods_configured = форма с производительми была отправлена)
if (isset($_GET['prods_configured'])) {
    $tmpCats = [];
    foreach ($getCats as $gp) {
        $cid = (int)($gp['id'] ?? 0);
        $cnm = trim($gp['name'] ?? '');
        if ($cid > 0 || $cnm !== '') $tmpCats[$cid] = $cnm ?: 'cat_'.$cid;
    }
    $ALLOWED_PRODUCERS = $tmpCats; // перезаписываем — даже пустым (все производителя)
}
...
if (!empty($ALLOWED_PRODUCERS)) {
    $producerParentIds = implode(',', array_keys($ALLOWED_PRODUCERS));
    $url .= '&producer_id=' . urlencode($producerParentIds);
}
...
if ($response === false || $httpCode !== 200) {
    ...
    } elseif (isset($data['producers']) && is_array($data['producers'])) {
...
if (!empty($producers) && !$fetchError) {
    // Fallback: if API returned all categories (no pagination support), slice locally
    if (count($producers) > $perPage) {
        $totalAll = count($producers);
        $offset = ($requestedPage - 1) * $perPage;
        $producers = array_slice($producers, $offset, $perPage);
        $totalItems = $totalAll;
        $totalPages = max(1, (int)ceil($totalAll / $perPage));
    }
    // Search filter by name (multi-term)
    if (!empty($getSearch)) {
        $producers = array_filter($producers, function($cat) use ($getSearch) {
            $n = $cat['producer_name'] ?? $cat['producer_title'] ?? '';
    ...
    if ($idMinGet > 0 || $idMaxGet > 0) {
        $producers = array_filter($producers, function($cat) use ($idMinGet, $idMaxGet) {
            $aid = (int)($cat['producer_id'] ?? 0);
    ...
    foreach ($producers as $a) {
        $producerParentId = (int)($a['producer_parent_id'] ?? 0);
        $producerName = $a['producer_name'] ?? '';
$id = (int)($a['producer_id'] ?? 0); $name = $a['producer_name'] ?? '';
$slug = $a['producer_name'] ?? ''; $language = $a['producer_lang'] ?? 'ru';
...
if ($importOnlyNamed && trim($name) === '') { $skipped++; continue; }
        $title = $a['producer_title_inpage'] ?? $a['producer_title'] ?? '';
        $metaTitle = $a['producer_title_seo_append'] ?? '';
        $metaDesc = $a['producer_meta_d'] ?? '';
        $metaKeywords = $a['producer_meta_k'] ?? '';
        $description = $a['producer_description_seo_append'] ?? '';
        $producerButton = $a['producer_button'] ?? '';
        $status = (int)($a['producer_access'] ?? 0);
        $priority = (int)($a['producer_position']??0);
        $subdomain = (int)($a['subdomain']??0);
        $view = (int)($a['producer_view']??0);
        $settingsComments = $a['producer_settings_comments']??'';
        $settingsTags = (int)($a['producer_settings_tags']??0);
        $comments = (int)($a['producer_comments']??0);
        $settingsRating = (int)($a['producer_settings_rating']??0);
        $showTree = (int)($a['producer_show_tree']??0);
        $showPeriod = (int)($a['producer_show_period']??0);
        $rating = (int)($a['producer_rating']??0);
        $datestamp = $a['producer_datestamp']??'';
        $multilangid = $a['producer_multilangid']??''; $tags = $a['tags']??'';
        $producerVisible = (int)($a['producer_visible']??1);
        $producerText = $a['producer_text'] ?? '';
        $producerImage = $a['producer_image'] ?? '';
        $producerButton = $a['producer_button'] ?? '';
        $producerShowTree = (int)($a['producer_show_tree']??0);
        $producerSettingsChild = (int)($a['producer_settings_childproductshow']??0);
        $producerSettingsStyle = (int)($a['producer_settings_style']??0);
        $producerSettingsSibling = (int)($a['producer_settings_sibling']??0);
        $producerGooglegoodsCatid = (int)($a['producer_googlegoods_catid']??0);
        $producerPromuaCatid = (int)($a['producer_promua_catid']??0);
        $producerRozetkauaTitle = $a['producer_rozetkaua_title']??'';
        $producerMerchantProductCategory = $a['producer_merchant_product_category']??'';
        $producerGmProductType = $a['producer_gm_product_type']??'';
        $producerGmSizetype = (int)($a['producer_gm_sizetype']??0);
        $producerPriceappendRozetka = $a['producer_priceappend_rozetka']??'';
        $producerPriceappendPromua = $a['producer_priceappend_promua']??'';
        $producerImageSettings = (int)($a['producer_image_settings']??0);
        $producerCountProducts = (int)($a['producer_count_products']??0);
        $producerCountSubcategories = (int)($a['producer_count_subcategories']??0);
        $producerLevel = (int)($a['producer_level']??0);
        $producerTree = $a['producer_tree']??'';
        $producerTextPosition = (int)($a['producer_text_position']??0);
        $producerAuthor = (int)($a['producer_author']??0);
        $producerShowInlist = (int)($a['producer_visible']??0);
        $subDir = '';
...
$filename = $id.($safeName!==''?'-'.$safeName:'').'-'.$language.'.html';
        ...
        $metaList = [
            'id'=>$id,'producer_id'=>$id,'name'=>$name,'slug'=>$slug,'producer_name'=>$producerName,
            'producer_title'=>$title,'producer_lang'=>$language,'producer_parent_id'=>$producerParentId,
            'producer_visible'=>$producerVisible,'producer_show_tree'=>$producerShowTree,
            'producer_access'=>$status,'producer_position'=>$priority,
            'producer_view'=>$view,'producer_rating'=>$rating,'producer_multilangid'=>$multilangid,
            'producer_image'=>$producerImage,'producer_image_settings'=>$producerImageSettings,
            'producer_level'=>$producerLevel,'producer_tree'=>$producerTree,
            'producer_text_position'=>$producerTextPosition,
            'producer_count_products'=>$producerCountProducts,'producer_count_subcategories'=>$producerCountSubcategories,
            'producer_title_inpage'=>$title,'producer_title_seo_append'=>$metaTitle,
            'producer_description_seo_append'=>$description,'producer_meta_d'=>$metaDesc,
            'producer_meta_k'=>$metaKeywords,
            'producer_settings_comments'=>$settingsComments,'producer_settings_tags'=>$settingsTags,
            'producer_comments'=>$comments,'producer_settings_rating'=>$settingsRating,
            'producer_settings_childproductshow'=>$producerSettingsChild,
            'producer_settings_style'=>$producerSettingsStyle,'producer_settings_sibling'=>$producerSettingsSibling,
            'producer_googlegoods_catid'=>$producerGooglegoodsCatid,
            'producer_promua_catid'=>$producerPromuaCatid,'producer_rozetkaua_title'=>$producerRozetkauaTitle,
            'producer_merchant_product_category'=>$producerMerchantProductCategory,
            'producer_gm_product_type'=>$producerGmProductType,'producer_gm_sizetype'=>$producerGmSizetype,
            'producer_priceappend_rozetka'=>$producerPriceappendRozetka,
            'producer_priceappend_promua'=>$producerPriceappendPromua,
            'producer_author'=>$producerAuthor,'tags'=>$tags,
        ...
        $h .= '<!-- PRODUCER TEXT BELOW -->'."\n".$producerText."\n";
        $h .= '<!-- PRODUCER BUTTON BELOW -->'."\n".$producerButton."\n";
        file_put_contents($filepath,$h); $saved++;
        $savedProducts[] = ['id'=>$id,'language'=>$language,'path'=>$relPath,'title'=>$title,'slug'=>$slug,
            'producer'=>$producerName,'status'=>$status,'descLen'=>mb_strlen($producerText),
...
?><!DOCTYPE html>
<html lang="ru">
<head><meta charset="UTF-8"><title>Импорт производителей — Boostore.pro</title>
<style>
...
<h1 data-i18n="import_results_title">▸ Импорт производителей — получено с API</h1>
<?php if ($fetchError): ?>
...
<div class="card" style="display:flex;gap:8px;align-items:center;margin-bottom:26px;flex-wrap:wrap;padding:10px;">
  ...
  <?php endif; ?>
  <span style="font-size:11px;color:#555;"><span data-i18n="per_page_by">по</span> <?=$perPage?> <span data-i18n="products_count">производителей</span>, <span data-i18n="all_total">всего</span> <?=$totalItems?></span>
</div>
<?php endif; ?>
<?php if($fixes):?>
...
<div style="font-size:12px;color:#e0e0e0;margin-bottom:8px;">Производители одной группы (одинаковый slug) имеют разный размер описания (расхождение > 50%):</div>
<table style="width:100%;font-size:12px;border-collapse:collapse;"><tr style="color:#ff9800;"><th style="padding:4px 8px;text-align:left;">slug</th><th style="padding:4px 8px;text-align:left;">мин</th><th style="padding:4px 8px;text-align:left;">макс</th><th style="padding:4px 8px;text-align:left;">производителя</th></tr>
...
<?php endforeach;?>
<div class="footer"><strong>Boostore.pro</strong> — <span data-i18n="import_complete">Импорт производителей завершён</span></div>
<script>
(function(){
...
<?php exit;
// ===================================================================
// _get-shop_producers_categories.php — END
// ===================================================================
endif;

// ===================================================================
// _update-shop_producers_categories.php — ИМПОРТ (отправка производителей на API)
// ===================================================================
if ($action === 'update'):
...
$expProductId = !empty($_GET['export_producer_id']) && $_GET['export_producer_id'] !== '0';
$expProducerId = !empty($_GET['export_producer_id']) && $_GET['export_producer_id'] !== '0';
$expProducerName = empty($_GET['export_producer_name']) || $_GET['export_producer_name'] !== '0'; // по умолчанию true
...
?><!DOCTYPE html>
<html lang="ru">
<head><meta charset="UTF-8"><title>Экспорт производителей — Boostore.pro</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}body{background:#1a1a2e;color:#e0e0e0;font-family:'Segoe UI',system-ui,sans-serif;padding:30px}.wrap{max-width:1200px;margin:0 auto}
...
<body><div class="wrap"><?php echo $header; ?>
<div class="plaque">
<span data-i18n="plaque_export">▸ <strong>Настройки экспорта</strong> — отправка производителей на Boostore.pro</span>
<span><a href="?site=<?=urlencode($currentSite)?>" style="padding:6px 16px;background:transparent;color:#00d4ff;border:1px solid #00d4ff;border-radius:4px;text-decoration:none;font-size:13px;" data-i18n="back_home">← Назад</a></span>
</div>
...
<script>
var _lang='ru';try{_lang=localStorage.getItem('boostore_lang')||navigator.language.slice(0,2);localStorage.setItem('boostore_lang',_lang);}catch(e){}if(!['ru','en','ua'].includes(_lang))_lang='ru';
var _t={ru:{'home':'🏠 Главная','site_label':'Сайт:','entity_name':'Производители','import':'📥 Импорт','export':'📤 Экспорт','completed':'✓ Результат','result_label':'✓ Результат','created_label':'✅ Создано:','updated_label':'📝 Обновлено:','errors_label':'❌ Ошибок:','skipped_exist_label':'⏭ Пропущено (существуют):','skipped_exist_label2':'⏭ Пропущено','export_text_only':'📝 Экспортировать только текст','file_selection':'📁 Выбор файлов','filter_name':'Фильтр по имени (slug)','batch_label':'Отправить за 1 раз','lang_label':'Язык','step_forward':'➡ ДАЛЕЕ','back_home':'← Назад','dry_run_label':'Dry run','plaque_export':'▸ <strong>Настройки экспорта</strong> — отправка производителей на Boostore.pro','dryrun_warn':'⚡ DRY RUN — запросы не отправляются','btn_more':'+ ЕЩЕ','btn_more_multi':'📋 ЕЩЕ НЕСКОЛЬКО','prompt_values':'Введите значения (каждая строка — отдельное поле):','search_placeholder':'часть имени, например: shopify','all_languages':'все','date_mode_meta':'Из мета-данных (дата из каждого производителя)','date_mode_fixed':'Одна дата для всех производителей','date_mode_offset':'Смещение дат (+N дней на производитель)','date_fixed_label':'📅 Фиксированная дата','date_offset_label':'📅 Базовая дата','date_offset_days':'+ дней на статью','override_planned':'📅 Переопределить planned','planned_notset':'— не указано (из мета-данных)','planned_0':'0 — не отложенная','planned_1':'1 — отложенная публикация','status_mode_label':'🔒 Статус доступа (status)','status_mode_meta':'Из мета-данных (статус из каждого производителя)','status_mode_override':'Переопределить для всех производителей','status_value_label':'Значение статуса','status_published':'1 — опубликовано (доступно)','status_hidden':'0 — скрыто (недоступно)','mode_label':'🔄 Режим экспорта','mode_all':'Добавление + обновление','mode_insert':'Только добавление новых','mode_update':'Только обновление существующих','fix_export_title':'🔧 Исправление при экспорте','fix_export_desc':'Синхронизировать поля с эталонным производителем (по slug) при экспорте','date_mode_label':'📅 Режим даты публикации','title':'Управление производителями магазина — Boostore.pro','fix_import_title':'🔧 Исправление по эталону','fix_import_desc':'Синхронизировать поля с эталонным производителем (по slug) после сохранения','ref_lang_label':'🌐 Язык эталонной производителя','export_fields_label':'📋 Поля для экспорта','export_producer_id_label':'ID виробника','export_product_id_label':'Product ID','export_producer_name_label':'Имя производителя','import_only_named':'Только с именем'},en:{'home':'🏠 Home','site_label':'Site:','entity_name':'Producers','import':'📥 Import','export':'📤 Export','completed':'✓ Completed','result_label':'✓ Result','created_label':'✅ Created:','updated_label':'📝 Updated:','errors_label':'❌ Errors:','skipped_exist_label':'⏭ Skipped (exist):','skipped_exist_label2':'⏭ Skipped','export_text_only':'📝 Export text only','file_selection':'📁 File selection','filter_name':'Filter by name (slug)','batch_label':'Send per run','lang_label':'Language','id_min_label':'ID >','id_max_label':'ID <','id_min_placeholder':'1000','id_max_placeholder':'5000','step_forward':'➡ NEXT','back_home':'← Back','dry_run_label':'Dry run','plaque_export':'▸ <strong>Export Settings</strong> — sending products to Boostore.pro','dryrun_warn':'⚡ DRY RUN — no API calls sent','btn_more':'+ MORE','btn_more_multi':'📋 ADD MULTIPLE','prompt_values':'Enter values (each line is a separate field):','search_placeholder':'part of name, e.g.: shopify','all_languages':'all','date_mode_meta':'From meta-data (date from each article)','date_mode_fixed':'Single date for all products','date_mode_offset':'Date offset (+N days per article)','date_fixed_label':'📅 Fixed Date','date_offset_label':'📅 Base Date','date_offset_days':'+ days per article','override_planned':'📅 Override planned','planned_notset':'— not set (from meta-data)','planned_0':'0 — not planned','planned_1':'1 — planned publishing','status_mode_label':'🔒 Access Status (status)','status_mode_meta':'From meta-data (status from each article)','status_mode_override':'Override for all products','status_value_label':'Status Value','status_published':'1 — published (public)','status_hidden':'0 — hidden (private)','mode_label':'🔄 Export mode','mode_all':'Add + Update','mode_insert':'Add new only','mode_update':'Update existing only','fix_export_title':'🔧 Fix on Export','fix_export_desc':'Sync fields with reference producer (by slug) on export','date_mode_label':'📅 Publication Date Mode','title':'Producer Management — Boostore.pro','fix_import_title':'🔧 Fix by Reference','fix_import_desc':'Sync fields with reference producer (by slug) after save','ref_lang_label':'🌐 Reference Language','export_fields_label':'📋 Fields to export','export_producer_id_label':'Producer ID','export_product_id_label':'Product ID','export_producer_name_label':'Producer name','import_only_named':'Named only'},ua:{'home':'🏠 Головна','site_label':'Сайт:','entity_name':'Виробники','import':'📥 Імпорт','export':'📤 Експорт','completed':'✓ Результат','result_label':'✓ Результат','created_label':'✅ Створено:','updated_label':'📝 Оновлено:','errors_label':'❌ Помилок:','skipped_exist_label':'⏭ Пропущено (існують):','skipped_exist_label2':'⏭ Пропущено','export_text_only':'📝 Експортувати тільки текст','file_selection':'📁 Вибір файлів','filter_name':'Фільтр за іменем (slug)','batch_label':'Відправити за 1 раз','lang_label':'Мова','step_forward':'➡ ДАЛІ','back_home':'← На головну','dry_run_label':'Dry run','plaque_export':'▸ <strong>Налаштування експорту</strong> — відправлення производителей на Boostore.pro','dryrun_warn':'⚡ DRY RUN — запити не надсилаються','btn_more':'+ ЩЕ','btn_more_multi':'📋 ДОДАТИ КІЛЬКА','prompt_values':'Введіть значення (кожен рядок — окреме поле):','search_placeholder':'частина імені, наприклад: shopify','all_languages':'всі','date_mode_meta':'З мета-даних (дата з кожної статті)','date_mode_fixed':'Одна дата для всіх производителей','date_mode_offset':'Зміщення дат (+N днів на статтю)','date_fixed_label':'📅 Фіксована дата','date_offset_label':'📅 Базова дата','date_offset_days':'+ днів на статтю','override_planned':'📅 Перевизначити planned','planned_notset':'— не вказано (з мета-даних)','planned_0':'0 — не відкладена','planned_1':'1 — відкладена публікація','status_mode_label':'🔒 Статус доступу (status)','status_mode_meta':'З мета-даних (статус з кожної статті)','status_mode_override':'Перевизначити для всіх производителей','status_value_label':'Значення статусу','status_published':'1 — опубліковано (доступно)','status_hidden':'0 — приховано (недоступно)','mode_label':'🔄 Режим експорту','mode_all':'Додавання + оновлення','mode_insert':'Тільки додавання нових','mode_update':'Тільки оновлення існуючих','fix_export_title':'🔧 Виправлення при експорті','fix_export_desc':'Синхронізувати поля з еталонним виробником (по slug) при експорті','date_mode_label':'📅 Режим дати публікації','title':'Керування виробниками магазину — Boostore.pro','fix_import_title':'🔧 Виправлення за еталоном','fix_import_desc':'Синхронізувати поля з еталонним виробником (по slug) після збереження','ref_lang_label':'🌐 Мова еталонного виробника','export_fields_label':'📋 Поля для експорту','export_producer_id_label':'ID виробника','export_product_id_label':'ID товару','export_producer_name_label':'Ім\'я виробника','import_only_named':'Тільки з назвою'}};
function applyLang(l){try{localStorage.setItem('boostore_lang',l);}catch(e){}_lang=l;document.querySelectorAll('[data-i18n]').forEach(function(el){var key=el.getAttribute('data-i18n');if(_t[l]&&_t[l][key]!==undefined)el.innerHTML=_t[l][key];});document.querySelectorAll('[data-i18n-placeholder]').forEach(function(el){var key=el.getAttribute('data-i18n-placeholder');if(_t[l]&&_t[l][key]!==undefined)el.placeholder=_t[l][key];});}
if(_lang!='ru'){document.addEventListener('DOMContentLoaded',function(){applyLang(_lang);});}
document.addEventListener('DOMContentLoaded',function(){var ls=document.getElementById('lang_switcher');if(ls){ls.value=_lang;ls.addEventListener('change',function(){applyLang(this.value);});}});
</script>
  ...
  <div style="display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap;">
    <div>
      ...
      <select name="date_mode" id="exp_date_mode" onchange="toggleExpDateFields()" style="padding:6px 8px;border:1px solid #0f3460;border-radius:4px;background:#16213e;color:#e0e0e0;">
        <option value=""<?=($_GET['date_mode']??$DATE_MODE)===''?' selected':''?> data-i18n="date_mode_meta">Из мета-данных (дата из каждого производителя)</option>
        <option value="fixed"<?=($_GET['date_mode']??$DATE_MODE)==='fixed'?' selected':''?> data-i18n="date_mode_fixed">Одна дата для всех производителей</option>
        <option value="offset"<?=($_GET['date_mode']??$DATE_MODE)==='offset'?' selected':''?> data-i18n="date_mode_offset">Смещение дат (+N дней на производитель)</option>
      </select>
    </div>
    ...
    <div id="exp_date_offset_days_block" style="display:<?=(($_GET['date_mode']??$DATE_MODE)==='offset')?'block':'none'?>;">
      <label style="font-size:11px;color:#888;display:block;margin-bottom:3px;" data-i18n="date_offset_days">+ дней на производитель</label>
    ...
    <div>
      ...
      <select name="status_mode" id="exp_status_mode" onchange="toggleExpStatusFields()" style="padding:6px 8px;border:1px solid #0f3460;border-radius:4px;background:#16213e;color:#e0e0e0;">
        <option value=""<?=($_GET['status_mode']??$STATUS_MODE)===''?' selected':''?> data-i18n="status_mode_meta">Из мета-данных (статус из каждого производителя)</option>
        <option value="override"<?=($_GET['status_mode']??$STATUS_MODE)==='override'?' selected':''?> data-i18n="status_mode_override">Переопределить для всех производителей</option>
      </select>
    </div>
  ...
  <div>
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;">
      <label style="font-size:12px;color:#ccc;cursor:pointer;display:flex;align-items:center;gap:3px;">
        ...
        <input type="checkbox" name="export_product_id" value="1"<?=!empty($_GET['export_producer_id'])?' checked':''?> data-i18n="export_product_id"> <span data-i18n="export_product_id_label">ID производителя</span>
      </label>
      <label style="font-size:12px;color:#ccc;cursor:pointer;display:flex;align-items:center;gap:3px;">
        <input type="hidden" name="export_producer_id" value="0">
        <input type="checkbox" name="export_producer_id" value="1"<?=!empty($_GET['export_producer_id'])?' checked':''?> data-i18n="export_producer_id"> <span data-i18n="export_producer_id_label">ID производителя</span>
      </label>
      <label style="font-size:12px;color:#ccc;cursor:pointer;display:flex;align-items:center;gap:3px;">
        <input type="hidden" name="export_producer_name" value="0">
        <input type="checkbox" name="export_producer_name" value="1"<?=(!isset($_GET['export_producer_name'])||$_GET['export_producer_name']!=='0')?' checked':''?> data-i18n="export_producer_name"> <span data-i18n="export_producer_name_label">Имя производителя</span>
      </label>
    </div>
  </div>
  <hr style="border-color:#0f3460;margin:4px 0;">
...
$htmlFiles2 = array_filter($htmlFiles2, function($p) { return !in_array(basename($p), ['index.php','_setting_shop_producers_categories.inc']); });
...
<!DOCTYPE html>
<html lang="ru">
<head><meta charset="UTF-8"><title data-i18n="step2_title">Экспорт производителей — выбор файлов</title>
...
var _t={ru:{'home':'🏠 Главная','site_label':'Сайт:','entity_name':'Производители','import':'📥 Импорт','export':'📤 Экспорт','completed':'✓ Результат','result_label':'✓ Результат','created_label':'✅ Создано:','updated_label':'📝 Обновлено:','errors_label':'❌ Ошибок:','skipped_exist_label':'⏭ Пропущено (существуют):','skipped_exist_label2':'⏭ Пропущено','export_text_only':'📝 Экспортировать только текст','file_selection':'📁 Выбор файлов','step2_title':'Экспорт производителей — выбор файлов','step2_header':'▸ <strong>Шаг 2</strong> — выберите файлы для экспорта на сайт','back_to_filters':'← Назад к фильтрам','no_files_found':'Нет файлов, соответствующих критериям поиска','files_found':'Найдено файлов:','select_all':'☑ ВЫДЕЛИТЬ ВСЕ','deselect_all':'☐ СНЯТЬ ВСЕ','export_selected':'📤 ЭКСПОРТИРОВАТЬ ВЫДЕЛЕННЫЕ','date_mode_label':'📅 Режим даты публикации','title':'Управление производителями магазина — Boostore.pro','fix_import_title':'🔧 Исправление по эталону','fix_import_desc':'Синхронизировать поля с эталонным производителем (по slug) после сохранения','ref_lang_label':'🌐 Язык эталонной производителя','export_fields_label':'📋 Поля для экспорта','export_producer_id_label':'ID виробника','export_product_id_label':'Product ID','export_producer_name_label':'Имя производителя','import_only_named':'Только с именем'},en:{'home':'🏠 Home','site_label':'Site:','entity_name':'Producers','import':'📥 Import','export':'📤 Export','completed':'✓ Completed','result_label':'✓ Result','created_label':'✅ Created:','updated_label':'📝 Updated:','errors_label':'❌ Errors:','skipped_exist_label':'⏭ Skipped (exist):','skipped_exist_label2':'⏭ Skipped','export_text_only':'📝 Export text only','file_selection':'📁 File selection','step2_title':'Export — file selection','step2_header':'▸ <strong>Step 2</strong> — select files to export','back_to_filters':'← Back to filters','no_files_found':'No files matching search criteria','files_found':'Files found:','select_all':'☑ SELECT ALL','deselect_all':'☐ DESELECT ALL','export_selected':'📤 EXPORT SELECTED','date_mode_label':'📅 Publication Date Mode','title':'Producer Management — Boostore.pro','fix_import_title':'🔧 Fix by Reference','fix_import_desc':'Sync fields with reference producer (by slug) after save','ref_lang_label':'🌐 Reference Language','export_fields_label':'📋 Fields to export','export_producer_id_label':'Producer ID','export_product_id_label':'Product ID','export_producer_name_label':'Producer name','import_only_named':'Named only'},ua:{'home':'🏠 Головна','site_label':'Сайт:','entity_name':'Виробники','import':'📥 Імпорт','export':'📤 Експорт','completed':'✓ Результат','result_label':'✓ Результат','created_label':'✅ Створено:','updated_label':'📝 Оновлено:','errors_label':'❌ Помилок:','skipped_exist_label':'⏭ Пропущено (існують):','skipped_exist_label2':'⏭ Пропущено','export_text_only':'📝 Експортувати тільки текст','file_selection':'📁 Вибір файлів','step2_title':'Експорт — вибір файлів','step2_header':'▸ <strong>Крок 2</strong> — виберіть файли для експорту','back_to_filters':'← Назад до фільтрів','no_files_found':'Немає файлів, що відповідають критеріям пошуку','files_found':'Знайдено файлів:','select_all':'☑ ВИДІЛИТИ ВСІ','deselect_all':'☐ ЗНЯТИ ВСІ','export_selected':'📤 ЕКСПОРТУВАТИ ВИДІЛЕНІ','date_mode_label':'📅 Режим дати публікації','title':'Керування виробниками магазину — Boostore.pro','fix_import_title':'🔧 Виправлення за еталоном','fix_import_desc':'Синхронізувати поля з еталонним виробником (по slug) після збереження','ref_lang_label':'🌐 Мова еталонного виробника','export_fields_label':'📋 Поля для експорту','export_producer_id_label':'ID виробника','export_product_id_label':'ID товару','export_producer_name_label':'Ім\'я виробника','import_only_named':'Тільки з назвою'}};
...
.prod-row{display:flex;gap:8px;margin-bottom:6px;align-items:center;flex-wrap:wrap}.prod-row input[type="checkbox"]{width:auto;cursor:pointer}.prod-row input[type="text"]{flex:1;min-width:60px}
...
<form method="get" action="?" id="export-step2">
  ...
  <input type="hidden" name="export_product_id" value="<?=(int)(!empty($_GET['export_producer_id'])?$_GET['export_producer_id']:0)?>">
  <input type="hidden" name="export_producer_id" value="<?=(int)(!empty($_GET['export_producer_id'])?$_GET['export_producer_id']:0)?>">
  <input type="hidden" name="export_producer_name" value="<?=(int)(empty($_GET['export_producer_name'])?1:$_GET['export_producer_name'])?>">
  ...
  <h3 style="margin:0 0 10px;font-size:15px;color:#4dc9f6;">📂 Производители для фильтрации</h3>
  <p style="font-size:11px;color:#888;margin-bottom:10px;">Если не выбрано ни одной — обрабатываются все производителя.</p>
  <div id="export-prods">
    <?php $epIdx=0; foreach($ALLOWED_PRODUCERS as $ecId=>$ecName): ?>
    <div class="prod-row">
      <input type="checkbox" name="export_cat[<?=$epIdx?>][checked]" value="1" checked style="width:auto;flex:0 0 20px;">
      <input type="text" name="export_cat[<?=$epIdx?>][id]" value="<?=$ecId?>" placeholder="ID" style="max-width:80px;padding:5px 8px;">
      <input type="text" name="export_cat[<?=$epIdx?>][name]" value="<?=htmlspecialchars($ecName)?>" placeholder="имя производителя" style="padding:5px 8px;">
  ...
  <input type="hidden" name="export_prods_configured" value="1">
  </div>
  <script>
  function addExportCatRow(){var c=document.getElementById('export-prods'),i=c.children.length;var d=document.createElement('div');d.className='prod-row';d.innerHTML="<input type='checkbox' name='export_cat["+i+"][checked]' value='1' checked style='width:auto;flex:0 0 20px;'><input type='text' name='export_cat["+i+"][id]' placeholder='ID' style='max-width:80px;padding:5px 8px;'><input type='text' name='export_cat["+i+"][name]' placeholder='имя производителя' style='padding:5px 8px;'><button type='button' onclick='this.parentElement.remove()' style='padding:3px 8px;font-size:11px;background:#f44336;color:#fff;border:none;border-radius:3px;cursor:pointer;'>✕</button>";c.appendChild(d);}
  </script>
...
function extractProducerContent(string $html):string{
    foreach (['<!-- PRODUCER TEXT BELOW -->'] as $sep) {
        $p=mb_strpos($html,$sep); if ($p!==false) { $c=mb_substr($html,$p+mb_strlen($sep)); $p2=mb_strpos($c,'<!-- PRODUCER BUTTON BELOW -->'); if($p2!==false) $c=mb_substr($c,0,$p2); return trim($c); }
    }
    $c=preg_replace('/<\/?body[^>]*>/i','',$html); $c=preg_replace('/<\/?html[^>]*>/i','',$c);
    return trim($c);
}
function extractProducerButton(string $html):string{
    foreach (['<!-- PRODUCER BUTTON BELOW -->'] as $sep) {
...
if (isset($_GET['files']) && is_array($_GET['files'])) {
    ...
    $htmlFiles=array_filter($htmlFiles,function($p){return!in_array(basename($p),['index.php','_setting_shop_producers_categories.inc']);});
...
if(empty($htmlFiles)):?><h1 data-i18n="no_html_files">✕ Нет HTML-файлов</h1><p data-i18n="no_html_files_desc">В папке <code><?=htmlspecialchars($CATEGORIES_DIR)?></code> не найдено *.html</p>
<?php else:
...
// Parse export category filter from step 2 (export_prods_configured), fallback to config
$activeCategories = $ALLOWED_PRODUCERS;
if (isset($_GET['export_prods_configured'])) {
    $exportCats = [];
...
$metaKeywords=$meta['meta_keywords']??'';$slug=$meta['slug']??$meta['producer_name']??'';$language=$meta['language']??$meta['producer_lang']??'ru';
$producerName=$meta['producer_name']??('sitecreate_'.$language);$producerParentId=(int)($meta['producer_id']??0);$producerParentId=(int)($meta['producer_parent_id']??0);
$producerButton=extractProducerButton($html);$status=$meta['status']??'';
...
$producerAllowed=empty($activeCategories);
if(!$producerAllowed&&$producerParentId>0&&isset($activeCategories[$producerParentId])){$producerAllowed=true;}
if(!$producerAllowed&&$producerParentId===0&&$producerName!==''){$f=array_search($producerName,$activeCategories,true);if($f!==false){$producerAllowed=true;}}
if(!$producerAllowed):$skippedCount++;?>
...
<div class="article-body"><div class="result-skip"><span style="color:#888;" data-i18n="skipped_producer">⏭ Пропущен — производитель не входит в ALLOWED_PRODUCERS</span><?php if($producerParentId):?><br><span class="inline-code">category_id: <?=$producerParentId?></span><?php endif;?><?php if($producerName):?><br><span class="inline-code">category_name: <?=htmlspecialchars($producerName)?></span><?php endif;?></div></div></div>
<?php continue; endif;
...
$producerText=extractProducerContent($html);
$payload=['producer_name'=>$slug,'producer_title'=>$title,'producer_lang'=>$language,'producer_parent_id'=>$producerParentId,'producer_visible'=>$producerVisible,'producer_text'=>$producerText,'producer_image'=>$producerImage,'producer_meta_d'=>$metaDesc,'producer_meta_k'=>$metaKeywords,'producer_title_inpage'=>$title,'producer_title_seo_append'=>$metaTitle,'producer_description_seo_append'=>$metaKeywords,'producer_button'=>$producerButton,'producer_settings_comments'=>$settingsComments,'producer_settings_tags'=>$settingsTags,'producer_comments'=>$comments,'producer_settings_rating'=>$settingsRating,'producer_password'=>$password,'producer_view'=>$view,'producer_rating'=>$rating,'producer_datestamp'=>$datestamp,'producer_multilangid'=>$multilangid,'tags'=>$tags,'subdomain'=>$subdomain,'slug'=>$slug];
...
if($articleId>0){
    $payload['producer_id'] = $articleId;
    $payload['id'] = $articleId;
}
if($expProducerId && $producerParentId>0){$payload['producers']=[['id'=>$producerParentId,'name'=>$producerName]];}
elseif($expProducerName && $producerName!==''){$payload['producers']=[['name'=>$producerName]];}
if($multilangid)$payload['multilangid']=$multilangid;
if($exportTextOnly){
    $keep = ['producer_id','id','update_exists','producer_name','producer_text','producer_button','producer_lang','slug'];
    $payload = array_intersect_key($payload, array_flip($keep));
}
$jsonPayload=json_encode($payload,JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);$descSize=mb_strlen($producerText);$metaCount=count($meta);
...
<details><summary data-i18n="description_title">📄 Description (полный текст)</summary><textarea readonly><?=htmlspecialchars($producerText)?></textarea></details>
...
$batchJson = json_encode(['producers' => $batchPayloads], JSON_UNESCAPED_UNICODE);
...
elseif($httpCode>=200&&$httpCode<300&&is_array($respData)):
    ...
    } elseif (isset($respData['producers']) && is_array($respData['producers'])) {
        $batchResults = [$respData['producers']];
    } elseif (isset($respData['products']) && is_array($respData['products'])) {
        $batchResults = [$respData['products']];
    }
    // Map results by slug_search or producer_name
    $resultMap = []; $resultIdx = 0;
    foreach ($batchResults as $br) {
        $slugFull = $br['slug_search'] ?? $br['producer_name'] ?? '';
        if ($slugFull) { 
    ...
    foreach ($batchProducts as $baIdx => $ba):
        ...
        <?php if ($hasErrors || empty($art)): ?>
            ...
            <?php if(empty($art)):?><br><span>Нет ответа для данной производителя</span><?php endif;?>
            </div>
            <?php endif; ?>
        <?php else: ?>
...
<?php exit;
// ===================================================================
// _update-shop_producers_categories.php — END
// ===================================================================
endif;
...
function saveConfigFromPost($post) {
    ...
    $producers = [];
    for ($i = 0; $i < count($cids); $i++) { $id = trim($cids[$i]); $nm = trim($cnms[$i]); if ($id !== '' || $nm !== '') $producers[(int)$id] = $nm; }
    $SITES[$ts]['allowed_producers'] = $producers;
    ...
    $SITES[$ts]['export_producer_id'] = isset($post['EXPORT_PRODUCT_ID']);
    $SITES[$ts]['export_producer_id'] = isset($post['EXPORT_CATEGORY_ID']);
    $SITES[$ts]['export_producer_name'] = !isset($post['EXPORT_CATEGORY_NAME']) || !empty($post['EXPORT_CATEGORY_NAME']);
    ...
    foreach ($SITES as $sd => $sc) {
        ...
        $c .= "  ".var_export($sd, true)." => ['key' => $sk, 'allowed_producers' => ";
        $acs = $sc['allowed_producers'] ?? [];
        if (empty($acs)) { $c .= "[]"; }
        ...
        $c .= ", 'export_producer_id' => ".($sc['export_producer_id']??false?'true':'false');
        $c .= ", 'export_producer_id' => ".($sc['export_producer_id']??false?'true':'false');
        $c .= ", 'export_producer_name' => ".($sc['export_producer_name']??true?'true':'false');
    ...
    $c .= "];\n";
    file_put_contents(__DIR__.'/_setting_shop_producers_categories.inc', $c);
}
$saveSuccess = false;
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_config'])) {
    saveConfigFromPost($_POST);
    $saveSuccess = true;
    require __DIR__.'/_setting_shop_producers_categories.inc';
    $currentSite = $_POST['site'] ?? (!empty($_GET['site']) ? $_GET['site'] : (array_keys($SITES)[0] ?? ''));
    $siteCfg = $SITES[$currentSite] ?? [];
    $ALLOWED_PRODUCERS  = $siteCfg['allowed_producers'] ?? [];
    ...
    $EXPORT_PRODUCT_ID   = $siteCfg['export_producer_id'] ?? false;
    $EXPORT_PRODUCER_ID  = $siteCfg['export_producer_id'] ?? false;
    $EXPORT_PRODUCER_NAME = $siteCfg['export_producer_name'] ?? true;
...
<html lang="ru">
<head>
<meta charset="UTF-8"><title>Управление производительми — Boostore.pro</title>
<style>
...
<strong>Boostore.pro</strong> — Скрипты для <strong>экспорта</strong> (скачивания) и <strong>импорта</strong> (отправки) производителей каталога через Commerce API. Сайт: <strong><?=htmlspecialchars($currentSite)?></strong>
</div>
<details class="card"><summary><span data-i18n="instr_title">📖 Инструкция</span> <span class="arrow">▶</span></summary><div class="card-body">
<p data-i18n="instr_intro">Все настройки — в разделе «Конфигурация» ниже. Если список производителей пуст — обрабатываются все производителя.</p>
...
<li data-i18n="step3">Выберите производителя производителей для работы</li>
<li data-i18n="step4">Нажмите <strong>"СКАЧАТЬ"</strong> — производителя скачаются в папку <code><?=htmlspecialchars($currentSite)?>/products/</code></li>
<li data-i18n="step5">При получении производителя одной группы (одинаковый slug) сверяются с эталонной версией (выбранный язык в конфигурации). <code>multilangid</code>, <code>stock_status</code>, <code>price</code>, <code>new</code>, <code>featured</code>, <code>promotion</code>, <code>price_old</code>, <code>status</code>, <code>datestamp</code> приводятся к эталону</li>
...
<h3 data-i18n="file_format">Формат файла</h3><p data-i18n="file_format_desc">Мета-данные и поля производителя в <code>&lt;meta name="..." content="..."&gt;</code> передают настройки производителя: slug, заголовок, язык, теги, дату публикации, статус доступа, категорию, цену, наличие, описание и системные параметры. Формат: текст производителя — после <code>&lt;!-- PRODUCER TEXT BELOW --&gt;</code>, кнопка — после <code>&lt;!-- PRODUCER BUTTON BELOW --&gt;</code></p>
<p style="color:#ff9800;font-size:12px;margin-top:6px;">💡 <strong>Удаление:</strong> добавьте <code>&lt;meta name="delete" content="true"&gt;</code> в HTML-файл и укажите <code>id</code> производителя — при экспорте запись будет удалена с сервера.</p>
</div></details>
...
<div style="font-size:12px;color:#888;margin-top:8px;" data-i18n="dryrun_desc">Режим «тест» — проверяет какие производителя будут отправлены, но сами запросы к API не выполняются.</div>
</div></div>
...
<hr><h3 data-i18n="cat_title">📂 Разрешённые производителя</h3><p style="color:#888;font-size:12px;margin-bottom:10px;" data-i18n="cat_desc">Только производителя этих производителей будут получены и отправлены. Если список пуст — обрабатываются все.</p>
<div style="display:grid;grid-template-columns:100px 1fr 40px;gap:8px;align-items:center;margin-bottom:6px;font-size:12px;color:#888;"><span data-i18n="cat_id_header">ID</span><span data-i18n="cat_name_header">Имя производителя</span><span></span></div>
<div id="cat-container"><?php $i=0;foreach($ALLOWED_PRODUCERS as $cid=>$cname):?>
<div class="form-row prod-row"><div class="field-sm"><input type="text" name="cat_id[]" value="<?=$cid?>"></div>
<div class="field"><input type="text" name="cat_name[]" value="<?=htmlspecialchars($cname)?>"></div>
<div style="flex:0 0 40px;text-align:center;"><button type="button" class="btn btn-danger btn-sm" onclick="this.closest('.prod-row').remove()">✕</button></div></div>
...
<div style="font-size:12px;color:#888;margin-bottom:8px;" data-i18n="import_settings_desc">Параметры, влияющие на получение и сохранение производителей из API</div>
...
<h3 style="margin:12px 0 8px;font-size:14px;color:#888;" data-i18n="products_count_title">📥 Количество производителей</h3>
...
<div style="font-size:11px;color:#888;margin-bottom:6px;" data-i18n="per_page_desc">Сколько производителей загружать за 1 запрос к API. Максимум 2000</div>
<input type="number" name="PER_PAGE" value="<?=(int)($PER_PAGE??200)?>" min="1" max="2000">
</div></div>
<h3 style="margin:12px 0 8px;font-size:14px;color:#888;" data-i18n="fix_export_title">🔧 Синхронизация полей производителя</h3>
<div class="form-row"><div class="field" style="max-width:250px;">
<label data-i18n="ref_lang_label">🌐 Язык эталонной производителя</label>
<div style="font-size:11px;color:#888;margin-bottom:6px;" data-i18n="ref_lang_desc">Производители этого языка считаются эталоном. При получении производителя других языков сверяются с ним</div>
...
<div style="font-size:12px;color:#888;margin-bottom:8px;" data-i18n="export_settings_desc">Параметры, влияющие на отправку производителей на сайт</div>
<div class="form-row"><div class="field" style="max-width:400px;">
<label data-i18n="date_mode_label">📅 Режим даты публикации</label>
<div style="font-size:11px;color:#888;margin-bottom:6px;" data-i18n="date_mode_desc">Как определять дату публикации производителей при импорте</div>
<select name="DATE_MODE" id="date_mode" onchange="toggleDateFields()">
<option value=""<?=$DATE_MODE===''?' selected':''?> data-i18n="date_mode_meta">Из мета-данных (дата из каждого производителя)</option>
<option value="fixed"<?=$DATE_MODE==='fixed'?' selected':''?> data-i18n="date_mode_fixed">Одна дата для всех производителей</option>
<option value="offset"<?=$DATE_MODE==='offset'?' selected':''?> data-i18n="date_mode_offset">Смещение дат (+N дней на производитель)</option>
</select>
</div></div>
...
<div style="font-size:11px;color:#888;margin-bottom:6px;" data-i18n="date_fixed_desc">Эта дата будет установлена всем производительм</div>
...
<div style="font-size:11px;color:#888;" data-i18n="date_offset_desc">Первая уникальная производитель (по slug) получит базовую дату, вторая — базовую дату + N дней, и т.д. Производители на разных языках с одинаковым slug считаются одной производителем и получают одинаковую дату.</div>
</div>
<script>
...
<div style="font-size:11px;color:#888;margin-bottom:6px;" data-i18n="status_mode_desc">Режим определения статуса публикации производителей при импорте</div>
<select name="STATUS_MODE" id="status_mode" onchange="toggleStatusFields()">
<option value=""<?=$STATUS_MODE===''?' selected':''?> data-i18n="status_mode_meta">Из мета-данных (статус из каждого производителя)</option>
<option value="override"<?=$STATUS_MODE==='override'?' selected':''?> data-i18n="status_mode_override">Переопределить для всех производителей</option>
</select>
</div></div>
...
<div style="font-size:11px;color:#888;margin-bottom:6px;" data-i18n="send_limit_desc">Сколько производителей можно отправить за один запуск. Максимум 5000</div>
...
<div style="font-size:11px;color:#888;margin-bottom:8px;" data-i18n="fix_desc">Какие поля автоматически исправлять по эталону при получении производителей</div>
...
<div class="footer"><strong>Boostore.pro</strong> — <span data-i18n="footer_text">Управление производителями каталога</span> &nbsp;|&nbsp; <a href="https://boostore.pro/ru/docs/api-integration/#hotengine-CommerceAPI" target="_blank" data-i18n="footer_docs">Документация API</a> &nbsp;|&nbsp; <a href="index.php" style="color:#00d4ff;" data-i18n="back_home">← На главную</a></div>
</div>
<script>
// ===== i18n Translations =====
var i18n = {
  ru: {
    'title':'Управление производителями магазина — Boostore.pro',
    ...
    'plaque':'Boostore.pro — Скрипты для экспорта (скачивания) и импорта (отправки) производителей каталога через Commerce API. Домен: ',
    'plaque_import':'▸ <strong>Настройки импорта</strong> — получение производителей с API',
    'plaque_export':'▸ <strong>Настройки экспорта</strong> — отправка производителей на Boostore.pro',
    'instr_title':'📖 Инструкция',
    'instr_intro':'Все настройки — в разделе «Конфигурация» ниже. Если список производителей пуст — обрабатываются все производителя.',
    ...
    'step3':'Выберите производителя производителей для работы',
    'step4':'Нажмите «ПОЛУЧИТЬ СТАТЬИ» — производителя скачаются в папку products/',
    'step5':'При получении производителя одной группы (одинаковый slug) сверяются с эталонной версией (выбранный язык в конфигурации). multilangid, stock_status, price, new, featured, promotion, price_old, status, datestamp приводятся к эталону',
    ...
    'file_format_desc':'Мета-данные в &lt;meta name=&quot;...&quot; content=&quot;...&quot;&gt; передают настройки производителя. Текст — после &lt;!-- PRODUCER TEXT BELOW --&gt;, кнопка — после &lt;!-- PRODUCER BUTTON BELOW --&gt;',
    'actions_title':'⚡ Действия',
    'btn_get':'📥 СКАЧАТЬ',
    'btn_update':'📤 ОТПРАВИТЬ',
    'btn_dryrun':'🔍 Тест (сухая отправка)',
    'dryrun_desc':'Режим «тест» — проверяет какие производителя будут отправлены, но сами запросы к API не выполняются.',
    ...
    'cat_title':'📂 Разрешённые производителя',
    'cat_desc':'Только производителя этих производителей будут получены и отправлены. Если список пуст — обрабатываются все.',
    'footer_text':'Управление производителями магазина',
    'footer_docs':'Документация API',
    'confirm_get':'Запустить получение производителей?',
    'confirm_update':'Запустить отправку производителей?',
    ...
    'date_mode_meta':'Из мета-данных (дата из каждого производителя)',
    'date_mode_fixed':'Одна дата для всех производителей',
    'date_mode_offset':'Смещение дат (+N дней на производитель)',
    'planned_notset':'— не указано (из мета-данных)',
    'planned_0':'0 — не отложенная',
    'planned_1':'1 — отложенная публикация',
    'status_mode_meta':'Из мета-данных (статус из каждого производителя)',
    'status_mode_override':'Переопределить для всех производителей',
    'export_settings_title':'📤 Настройки экспорта',
    'export_settings_desc':'Параметры, влияющие на получение и сохранение производителей из API',
    'folder_structure':'📁 Структура папок',
    'folder_planned':'Разделять planned в <code>products/planned/</code>',
    'folder_producer':'Разделять по папкам производителей',
    'products_count_title':'📥 Количество производителей',
    'per_page_label':'Производителей за запрос (per_page)',
    'per_page_desc':'Сколько производителей загружать за 1 запрос к API. Максимум 2000',
    'fix_export_title':'🔧 Исправление при экспорте',
    'ref_lang_label':'🌐 Язык эталонной производителя',
    'ref_lang_desc':'Производители этого языка считаются эталоном. При получении производителя других языков сверяются с ним',
    'import_settings_title':'📥 Настройки импорта',
    'import_settings_desc':'Параметры, влияющие на отправку производителей на сайт',
    'date_mode_label':'📅 Режим даты публикации',
    'date_mode_desc':'Как определять дату публикации производителей при импорте',
    'date_fixed_label':'📅 Фиксированная дата',
    'date_fixed_desc':'Эта дата будет установлена всем производительм',
    'date_offset_label':'📅 Базовая дата (от которой начинать отсчёт)',
    'date_offset_days':'+ дней на статью',
    'date_offset_desc':'Первая уникальная производитель (по slug) получит базовую дату, вторая — базовую дату + N дней, и т.д. Производители на разных языках с одинаковым slug считаются одной производителем и получают одинаковую дату.',
    'override_planned':'📅 Переопределить planned',
    'override_planned_desc':'Если задать — будет применено ко ВСЕМ производителям при импорте. Пусто — брать из мета-данных каждого производителя',
    'status_mode_label':'🔒 Статус доступа (status)',
    'status_mode_desc':'Режим определения статуса публикации производителей при импорте',
    ...
    'send_limit_desc':'Сколько производителей можно отправить за один запуск. Максимум 5000',
    'fix_title':'🔄 Исправление по эталону',
    'fix_desc':'Какие поля автоматически исправлять по эталону при получении производителей',
    'back_home':'Назад',
    'btn_more':'+ ЕЩЕ',
    ...
    'folder_producer_chk':'Разделять по папкам производителей',
    'cat_id_header':'ID',
    'cat_name_header':'Имя производителя',
    'fix_multilangid':'multilangid',
    'fix_planned':'planned',
    'fix_status':'status',
    'fix_datestamp':'datestamp',
    'import_only_named':'Только с именем (пропускать без name)',
    ...
    'cat_name_placeholder':'имя производителя',
    ...
    'home':'🏠 Главная',
    'entity_name':'Производители',
    'site_label':'Сайт:',
    'delete_site':'🗑 Удалить настройки сайта',
  },
  en: {
    'title':'Producer Management — Boostore.pro',
    ...
    'file_format_desc':'Meta data in &lt;meta name=&quot;...&quot; content=&quot;...&quot;&gt; carries producer settings. Text after &lt;!-- PRODUCER TEXT BELOW --&gt;, button after &lt;!-- PRODUCER BUTTON BELOW --&gt;',
    ...
    'folder_producer':'Separate by category folders',
    ...
    'folder_producer_chk':'Separate by category folders',
    'cat_id_header':'ID',
...
'home':'🏠 Home',
  ...
  ua: {
    ...
    'plaque':'Boostore.pro — Скрипти для експорту (завантаження) та імпорту (відправлення) производителей блогу через Commerce API. Домен: ',
    'plaque_import':'▸ <strong>Налаштування імпорту</strong> — отримання производителей з API',
    'plaque_export':'▸ <strong>Налаштування експорту</strong> — відправлення производителей на Boostore.pro',
    ...
    'step3':'Виберіть категорії производителей для роботи',
    ...
    'file_format_desc':'Мета-дані в &lt;meta name=&quot;...&quot; content=&quot;...&quot;&gt; передають налаштування виробника. Текст — після &lt;!-- PRODUCER TEXT BELOW --&gt;, кнопка — після &lt;!-- PRODUCER BUTTON BELOW --&gt;',
    'actions_title':'⚡ Дії',
    ...
    'confirm_get':'Запустити отримання производителей?',
    'confirm_update':'Запустити відправлення производителей?',
    ...
    'date_mode_fixed':'Одна дата для всіх производителей',
    ...
    'status_mode_override':'Перевизначити для всіх производителей',
    'export_settings_title':'📤 Налаштування експорту',
    'export_settings_desc':'Параметри, що впливають на отримання та збереження производителей з API',
    'folder_structure':'📁 Структура папок',
    'folder_planned':'Розділяти planned у <code>products/planned/</code>',
    'folder_producer':'Розділяти по папках категорій',
    'products_count_title':'📥 Кількість производителей',
    'per_page_label':'Статей за запит (per_page)',
    'per_page_desc':'Скільки производителей завантажувати за 1 запит до API. Максимум 2000',
    'fix_export_title':'🔧 Виправлення при експорті',
    'ref_lang_label':'🌐 Мова еталонної статті',
    'ref_lang_desc':'Статті цієї мови вважаються еталоном. При отриманні производителей іншими мовами звіряються з ним',
    'import_settings_title':'📥 Налаштування імпорту',
    'import_settings_desc':'Параметри, що впливають на відправлення производителей на сайт',
    'date_mode_label':'📅 Режим дати публікації',
    'date_mode_desc':'Як визначати дату публікації производителей при імпорті',
    'date_fixed_label':'📅 Фіксована дата',
    'date_fixed_desc':'Ця дата буде встановлена для всіх производителей',
    ...
    'override_planned_desc':'Якщо задано — застосовується до ВСІХ производителей при імпорті. Порожньо — брати з мета-даних кожної статті',
    'status_mode_label':'🔒 Статус доступу (status)',
    'status_mode_desc':'Режим визначення статусу публікації производителей при імпорті',
    ...
    'send_limit_desc':'Скільки производителей можна відправити за один запуск. Максимум 5000',
    'fix_title':'🔄 Виправлення за еталоном',
    'fix_desc':'Які поля автоматично виправляти за еталоном при отриманні производителей',
    ...
    'folder_producer_chk':'Розділяти по папках категорій',
    'cat_id_header':'ID',
...
function addCatRow(){var d=document.getElementById('cat-container'),div=document.createElement('div');div.className='form-row prod-row';div.innerHTML='<div class="field-sm"><input type="text" name="cat_id[]" value="" placeholder="ID" data-i18n-placeholder="cat_id_placeholder"></div><div class="field"><input type="text" name="cat_name[]" value="" placeholder="name" data-i18n-placeholder="cat_name_placeholder"></div><div style="flex:0 0 40px;text-align:center;"><button type="button" class="btn btn-danger btn-sm" onclick="this.closest(\'.prod-row\').remove()">✕</button></div>';d.appendChild(div);}
