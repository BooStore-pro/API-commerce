...
// ===================================================================
// Инструкция: https://boostore.pro/ru/docs/api-integration/#api-blocks
// ===================================================================

// Auto-create config file if not exists
$configFile = __DIR__ . '/_setting_blocks.inc';
if (!file_exists($configFile)) {
    $defaultConfig = "<?php
// ===================================================================
// НАСТРОЙКИ ИМПОРТА/ЭКСПОРТА БЛОКОВ/МЕНЮ (Blocks Boostore.pro)
// ===================================================================
\$SITES = [
    'site.boostore.pro' => [
        'key' => '',
...
$API_URL = 'https://' . $currentSite . '/api/commerce/blocks';

// Site directory
$SITE_DIR = __DIR__ . DIRECTORY_SEPARATOR . $currentSite;
$BLOCKS_DIR = $SITE_DIR . DIRECTORY_SEPARATOR . 'blocks';
if (!is_dir($BLOCKS_DIR)) { @mkdir($BLOCKS_DIR, 0777, true); }

// Per-site config
$siteCfg = $SITES[$currentSite] ?? [];
$PER_PAGE            = $siteCfg['per_page'] ?? ($PER_PAGE ?? 200);
$SEND_BATCH_LIMIT    = $siteCfg['send_batch_limit'] ?? ($SEND_BATCH_LIMIT ?? 100);
...
function sitesExport($sites) {
    ...
    foreach ($sites as $sDomain => $sCfg) {
        ...
        $c .= "        'send_batch_limit' => ".(int)($sCfg['send_batch_limit'] ?? 100).",\n";
        $c .= "    ],\n";
    }
    return $c . "]\n";
}
...
// ---- Хлебные крошки ----
$crumbs = ['<a href="index.php" style="color:#00d4ff;text-decoration:none;font-weight:600;"><span data-i18n="home">🏠 Главная</span></a>', '<a href="?site='.urlencode($currentSite).'" style="color:#00d4ff;text-decoration:none;font-weight:600;">🧩 <span data-i18n="entity_name">Блоки</span></a>'];
...
<h1 style="margin:0 0 20px 0;">▸ <span data-i18n="title">Управление блоками/меню — Boostore.pro</span></h1>
<div style="display:flex;gap:8px;align-items:center;">
'.(file_exists(__DIR__.'/_active_site.inc')
? '<strong style="font-size:14px;color:#ce93d8;font-family:monospace;">'.htmlspecialchars($currentSite).'</strong>'
...
// ===================================================================
// _get-blocks.php — ИМПОРТ (получение страниц с API)
// ===================================================================
if ($action === 'get'):
@set_time_limit(300);
...
<option value="pl"<?=($_GET['lang']??'')==='ru'?' selected':''?> data-i18n="lang_pl">Polski</option>
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:8px;">
    <div>
      ...
      <select name="import_ref_lang" style="padding:6px 8px;border:1px solid #0f3460;border-radius:4px;background:#0d1b2a;color:#e0e0e0;">
        <option value="be"<?=(($_GET['import_ref_lang']??$REFERENCE_LANG)??'ru')==='be'?' selected':''?>>Белорусский (be)</option>
        <option value="de"<?=(($_GET['import_ref_lang']??$REFERENCE_LANG)??'ru')==='de'?' selected':''?>>Немецкий (de)</option>
        <option value="en"<?=(($_GET['import_ref_lang']??$REFERENCE_LANG)??'ru')==='en'?' selected':''?>>English (en)</option>
        <option value="es"<?=(($_GET['import_ref_lang']??$REFERENCE_LANG)??'ru')==='es'?' selected':''?>>Испанский (es)</option>
        <option value="fr"<?=(($_GET['import_ref_lang']??$REFERENCE_LANG)??'ru')==='fr'?' selected':''?>>Français (fr)</option>
        <option value="it"<?=(($_GET['import_ref_lang']??$REFERENCE_LANG)??'ru')==='it'?' selected':''?>>Italiano (it)</option>
        <option value="kk"<?=(($_GET['import_ref_lang']??$REFERENCE_LANG)??'ru')==='kk'?' selected':''?>>Қазақ (kk)</option>
        <option value="pl"<?=(($_GET['import_ref_lang']??$REFERENCE_LANG)??'ru')==='ru'?' selected':''?>>Polski (pl)</option>
        <option value="ru"<?=(($_GET['import_ref_lang']??$REFERENCE_LANG)??'ru')==='ru'?' selected':''?>>Русский (ru)</option>
        <option value="ua"<?=(($_GET['import_ref_lang']??$REFERENCE_LANG)??'ru')==='ua'?' selected':''?>>Українська (ua)</option>
      </select>
    </div>
  </div>
...
if ($blocks !== null && !$fetchError) {
    ...
    if ($idMinGet > 0 || $idMaxGet > 0) {
        $blocks = array_filter($blocks, function($art) use ($idMinGet, $idMaxGet) {
            $aid = (int)($art['id'] ?? 0);
...
$slug = preg_replace('/-(ua|pl|en|ru)$/i', '', $slug);
        ...
        $position = $a['position'] ?? '';
        $menuOrder = $a['menu_order'] ?? $a['priority'] ?? 0;
        $show = $a['show'] ?? $a['status'] ?? 1;
        $showOnPage = $a['show_on_page'] ?? 0;
        $showOnDevice = $a['show_on_device'] ?? 0;
        $access = $a['access'] ?? 0;
        $qsShow = $a['querystring_show'] ?? '';
        $qsHide = $a['querystring_hide'] ?? '';
        $qspShow = $a['querystrpos_show'] ?? '';
        $qspHide = $a['querystrpos_hide'] ?? '';
        $script = $a['script'] ?? 0;
        $scriptAsync = $a['script_async'] ?? 0;
        $div = $a['div'] ?? 0;
        $safeName = preg_replace('/[^a-zA-Z0-9_-]/','',$name); $safeName = trim($safeName,'-_');
        if ($safeName==='') $safeName = (string)$id;
        $filename = $slug.'-'.$safeName.'-'.$language.'.html';
        $filepath = $baseDir.DIRECTORY_SEPARATOR.$filename; $relPath = $currentSite.DIRECTORY_SEPARATOR.'blocks'.DIRECTORY_SEPARATOR.$filename;
        $h = '<meta name="title" content="'.htmlspecialchars($title, ENT_QUOTES).'">'."\n";
        $metaList = [
            'id'=>$id,'name'=>$name,'slug'=>$slug,'language'=>$language,
            'position'=>$position,'menu_order'=>$menuOrder,'show'=>$show,
            'show_on_page'=>$showOnPage,'show_on_device'=>$showOnDevice,'access'=>$access,
            'script'=>$script,'script_async'=>$scriptAsync,'div'=>$div,
        ];
        foreach (['querystring_show','querystring_hide','querystrpos_show','querystrpos_hide'] as $k) {
            if (!empty($$k)) $metaList[$k] = $$k;
        }
...
<div class="card" style="display:flex;gap:8px;align-items:center;margin-bottom:26px;flex-wrap:wrap;padding:10px;">
  ...
  <?php endif; ?>
  <span style="font-size:11px;color:#555;"><span data-i18n="per_page_by">по</span> <?=$perPage?> <span data-i18n="blocks_count">страниц</span>, <span data-i18n="all_total">всего</span> <?=$totalItems?></span>
</div>
<?php endif; ?>
<?php if($fixes):?>
...
// ===================================================================
// _update-blocks.php — ЭКСПОРТ (отправка страниц на API)
// ===================================================================
if ($action === 'update'):
...
if ($expDateMode === 'offset' && $expDateOffsetBase !== '' && $expDateOffsetDays > 0) {
    ...
    if ($baseTs) {
        $blocksDir2 = $BLOCKS_DIR;
        if (is_dir($blocksDir2)) {
            $rdi2 = new RecursiveDirectoryIterator($blocksDir2,RecursiveDirectoryIterator::SKIP_DOTS);
...
$htmlFiles2 = array_filter($htmlFiles2, function($p) { return !in_array(basename($p), ['index.php','_setting_blocks.inc']); });
...
if (isset($_GET['files']) && is_array($_GET['files'])) {
    ...
    if(is_dir($blocksDir)){$rdi=new RecursiveDirectoryIterator($blocksDir,RecursiveDirectoryIterator::SKIP_DOTS);$rii=new RecursiveIteratorIterator($rdi);foreach($rii as $f){if($f->isFile()&&strtolower($f->getExtension())==='html')$htmlFiles[]=$f->getPathname();}sort($htmlFiles);}
    $skipFiles=['index.php','_setting_blocks.inc'];$skipNames=['llm','webmanifest','robots'];$htmlFiles=array_filter($htmlFiles,function($p)use($skipFiles,$skipNames){$bn=basename($p);if(in_array($bn,$skipFiles))return false;$fn=pathinfo($bn,PATHINFO_FILENAME);if(in_array(strtolower($fn),$skipNames))return false;return true;});
...
if(empty($htmlFiles)):?><h1 data-i18n="no_html_files">✕ Нет HTML-файлов</h1><p data-i18n="no_html_files_desc">В папке <code><?=htmlspecialchars($BLOCKS_DIR)?></code> не найдено *.html</p>
<?php // Note: $BLOCKS_DIR variable used above
else:
...
$title=$meta['title']??'';$slug=$meta['slug']??'';$language=$meta['language']??'ru';
if(in_array(strtolower($slug),['llm','webmanifest','robots'])){$skippedCount++;continue;}
$position=$meta['position']??'';$menuOrder=(int)($meta['menu_order']??0);
$show=(int)($meta['show']??1);$showOnPage=(int)($meta['show_on_page']??0);
$showOnDevice=(int)($meta['show_on_device']??0);$access=(int)($meta['access']??0);
$qsShow=$meta['querystring_show']??'';$qsHide=$meta['querystring_hide']??'';
$qspShow=$meta['querystrpos_show']??'';$qspHide=$meta['querystrpos_hide']??'';
$script=(int)($meta['script']??0);$scriptAsync=(int)($meta['script_async']??0);
$div=(int)($meta['div']??0);
$description=extractContent($html);
$blockId = (int)($meta['id'] ?? 0);
$doDelete = (isset($meta['delete']) && strtolower($meta['delete']) === 'true');
$payload = ['name'=>$slug, 'slug_search'=>$slug, 'language'=>$language, 'description'=>$description, 'update_exists'=>true];
if (!empty($title)) $payload['title'] = $title;
if ($position !== '') $payload['position'] = $position;
if ($menuOrder) $payload['menu_order'] = (int)$menuOrder;
if ($show !== '') $payload['show'] = (int)$show;
if ($showOnPage) $payload['show_on_page'] = (int)$showOnPage;
if ($showOnDevice) $payload['show_on_device'] = (int)$showOnDevice;
if ($access) $payload['access'] = (int)$access;
if (!empty($qsShow)) $payload['querystring_show'] = $qsShow;
if (!empty($qsHide)) $payload['querystring_hide'] = $qsHide;
if (!empty($qspShow)) $payload['querystrpos_show'] = $qspShow;
if (!empty($qspHide)) $payload['querystrpos_hide'] = $qspHide;
if ($script) $payload['script'] = (int)$script;
if ($scriptAsync) $payload['script_async'] = (int)$scriptAsync;
if ($div) $payload['div'] = (int)$div;
if ($doDelete && $blockId) { $payload['delete'] = true; $payload['id'] = $blockId; }
...
elseif($httpCode>=200&&$httpCode<300&&is_array($respData)):
    ...
    } elseif (isset($respData['blocks']) && is_array($respData['blocks'])) {
        $batchResults = [$respData['blocks']];
    }
...
function saveConfigFromPost($post) {
    ...
    $SITES[$ts]['reference_lang'] = $post['REFERENCE_LANG'] ?? 'pl';
    ...
    foreach ($SITES as $sd => $sc) {
        ...
        $c .= ", 'reference_lang' => ".var_export($sc['reference_lang']??'pl',true);
        $c .= ", 'fix_status' => ".($sc['fix_status']??false?'true':'false');
        $c .= "],\n";
    }
    $c .= "];\n";
    file_put_contents(__DIR__.'/_setting_blocks.inc', $c);
}
$saveSuccess = false;
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_config'])) {
    saveConfigFromPost($_POST);
    $saveSuccess = true;
    require __DIR__.'/_setting_blocks.inc';
...
<html lang="ru">
<head>
<meta charset="UTF-8"><title>Управление блоками/меню — Boostore.pro</title>
<style>
...
<li data-i18n="step4">Нажмите <strong>"СКАЧАТЬ"</strong> — страницы скачаются в папку <code><?=htmlspecialchars($currentSite)?>/blocks/</code></li>
...
<li data-i18n="step6">Отредактируйте HTML-файлы в <code><?=htmlspecialchars($currentSite)?>/blocks/</code> при необходимости</li>
...
<h3 data-i18n="file_format">Формат файла</h3><p data-i18n="file_format_desc">Мета-данные в <code>&lt;meta name="..." content="..."&gt;</code> передают настройки блока: имя, заголовок, язык, позицию, видимость, скрипты и т.д. Содержимое — после <code>&lt;!-- CONTENT SEPARATOR BELOW --&gt;</code></p>
<p style="color:#ff9800;font-size:12px;margin-top:6px;">💡 <strong>Удаление:</strong> добавьте <code>&lt;meta name="delete" content="true"&gt;</code> в HTML-файл и укажите <code>id</code> блока — при экспорте запись будет удалена с сервера.</p>
</div></details>
...
<div class="footer"><strong>Boostore.pro</strong> — <span data-i18n="footer_text">Управление блоками/меню</span> &nbsp;|&nbsp; <a href="https://boostore.pro/ru/docs/api-integration/#hotengine-CommerceAPI" target="_blank" data-i18n="footer_docs">Документация API</a> &nbsp;|&nbsp; <a href="index.php" style="color:#00d4ff;" data-i18n="back_home">← На главную</a></div>
</div>
<script>
// ===== i18n Translations =====
var i18n = {
  ru: {
    'title':'Управление блоками/меню — Boostore.pro',
    ...
    'step4':'Нажмите «СКАЧАТЬ» — страницы скачаются в папку blocks/',
    ...
    'step6':'Отредактируйте HTML-файлы в blocks/ при необходимости',
    ...
    'footer_text':'Управление блоками/меню',
  ...
  en: {
    'title':'Blocks Management — Boostore.pro',
    ...
    'plaque':'Boostore.pro — Scripts for exporting (downloading) and importing (uploading) blocks via Commerce API. Domain: ',
    'plaque_import':'▸ <strong>Import Settings</strong> — fetching blocks from API',
    'plaque_export':'▸ <strong>Export Settings</strong> — sending blocks to Boostore.pro',
    ...
    'step4':'Click «START IMPORT» — blocks will be downloaded to the blocks/ folder',
    ...
    'step6':'Edit HTML files in blocks/ if needed',
    ...
    'dryrun_desc':'Test mode — checks which blocks will be sent, but no actual API requests are made.',
    ...
    'confirm_get':'Start fetching blocks?',
    'confirm_update':'Start sending blocks?',
    ...
    'date_mode_fixed':'Single date for all blocks',
    'date_mode_offset':'Date offset (+N days per page)',
    'status_mode_meta':'From meta-data (status from each page)',
    'status_mode_override':'Override for all blocks',
    'export_settings_title':'📤 Export Settings',
    'export_settings_desc':'Parameters affecting fetching and saving blocks from the API',
    'per_page_label':'Pages per request (per_page)',
    'per_page_desc':'How many blocks to load per API request. Max 2000',
    'fix_export_title':'🔧 Fix on Export',
    'ref_lang_label':'🌐 Reference Language',
    'ref_lang_desc':'Pages in this language are considered reference. When fetching blocks in other languages, they are checked against it',
    'import_settings_title':'📥 Import Settings',
    'import_settings_desc':'Parameters affecting fetching and saving blocks from the API',
    ...
    'date_fixed_desc':'This date will be set for all blocks',
    ...
    'send_limit_desc':'How many blocks can be sent in one run. Max 5000',
    'fix_title':'🔄 Fix by Reference',
    'fix_desc':'Which fields to auto-fix by reference when fetching blocks',
    'back_home':'Home',
    'btn_more':'+ MORE',
...
'home':'🏠 Home',
  ...
  ua: {
    ...
    'step4':'Натисніть «ОТРИМАТИ СТОРІНКИ» — сторінки завантажаться в папку blocks/',
    ...
    'step6':'Відредагуйте HTML-файли в blocks/ за потреби',
