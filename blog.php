...
// ---- Хлебные крошки ----
$crumbs = ['<a href="index.php" style="color:#00d4ff;text-decoration:none;font-weight:600;"><span data-i18n="home">🏠 Главная</span></a>', '<a href="?site='.urlencode($currentSite).'" style="color:#00d4ff;text-decoration:none;font-weight:600;">📝 <span data-i18n="entity_name">Статьи</span></a>'];
...
$filename = $id.($safeName!==''?'-'.$safeName:'').'-'.$language.'.html';
        ...
        $metaList = [
            ...
            'status'=>$status,'category_id'=>$catId,'category_name'=>$categoryName,
...
<form method="get" action="?" id="export-step1" style="display:flex;flex-direction:column;gap:12px;">
  ...
  <div style="display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap;margin:6px 0;">
    <div><label style="font-size:11px;color:#888;display:block;margin-bottom:3px;" data-i18n="id_min_label">ID ></label><input type="number" name="id_min" value="<?=htmlspecialchars($_GET['id_min']??'')?>" min="0" placeholder="1000" style="width:90px;padding:6px 8px;border:1px solid #0f3460;border-radius:4px;background:#16213e;color:#e0e0e0;"></div>
    <div><label style="font-size:11px;color:#888;display:block;margin-bottom:3px;" data-i18n="id_max_label">ID <</label><input type="number" name="id_max" value="<?=htmlspecialchars($_GET['id_max']??'')?>" min="0" placeholder="5000" style="width:90px;padding:6px 8px;border:1px solid #0f3460;border-radius:4px;background:#16213e;color:#e0e0e0;"></div>
  </div>
  ...
  <div>
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;">
      <label style="font-size:12px;color:#ccc;cursor:pointer;display:flex;align-items:center;gap:3px;">
        ...
        <input type="checkbox" name="export_article_id" value="1"<?=!empty($_GET['export_article_id'])?' checked':''?> data-i18n="export_article_id"> <span data-i18n="export_article_id_label">ID статьи</span>
      </label>
      <label style="font-size:12px;color:#ccc;cursor:pointer;display:flex;align-items:center;gap:3px;">
...
<?php exit; endif; ?>
<?php endif; // end step 1 header wrapper ?>
<?php
...
// Apply ID range filter for step 2
$expIdMin2 = isset($_GET['id_min']) ? (int)$_GET['id_min'] : 0;
$expIdMax2 = isset($_GET['id_max']) ? (int)$_GET['id_max'] : 0;
if ($expIdMin2 > 0 || $expIdMax2 > 0) {
    $htmlFiles2 = array_filter($htmlFiles2, function($fp) use ($expIdMin2, $expIdMax2) {
        $h = @file_get_contents($fp);
        if ($h === false) return true;
        preg_match('/<meta\s+name=["\']id["\']\s+content=["\'](\d+)["\']/is', $h, $m);
        $aid = isset($m[1]) ? (int)$m[1] : 0;
        if ($aid === 0) { preg_match('/\/(\d+)[-\/]/', $fp, $m2); $aid = isset($m2[1]) ? (int)$m2[1] : 0; }
        if ($expIdMin2 > 0 && $aid < $expIdMin2) return false;
        if ($expIdMax2 > 0 && $aid > $expIdMax2) return false;
        return true;
    });
    $htmlFiles2 = array_values($htmlFiles2);
}
...
<form method="get" action="?" id="export-step2">
  ...
  <?php if (!empty($_GET['id_min'])): ?><input type="hidden" name="id_min" value="<?=(int)$_GET['id_min']?>"><?php endif; ?>
  <?php if (!empty($_GET['id_max'])): ?><input type="hidden" name="id_max" value="<?=(int)$_GET['id_max']?>"><?php endif; ?>
...
$expIdMin = isset($_GET['id_min']) ? (int)$_GET['id_min'] : 0;
$expIdMax = isset($_GET['id_max']) ? (int)$_GET['id_max'] : 0;
if ($expIdMin > 0 || $expIdMax > 0) {
    $htmlFiles = array_filter($htmlFiles, function($fp) use ($expIdMin, $expIdMax) {
        $h = @file_get_contents($fp);
        if ($h === false) return true;
        preg_match('/<meta\s+name=["\']id["\']\s+content=["\'](\d+)["\']/is', $h, $m);
        $aid = isset($m[1]) ? (int)$m[1] : 0;
        if ($aid === 0) { preg_match('/\/(\d+)[-\/]/', $fp, $m2); $aid = isset($m2[1]) ? (int)$m2[1] : 0; }
        if ($expIdMin > 0 && $aid < $expIdMin) return false;
        if ($expIdMax > 0 && $aid > $expIdMax) return false;
        return true;
    });
    $htmlFiles = array_values($htmlFiles);
}
...
<?php continue; endif;
if($expStatusMode==='override'){$status=$expStatusOverride;}elseif($status===''||$status===null){$status=1;}else{$status=(int)$status;}
...
$_mcPos = mb_strpos($description, '<!-- MAIN CONTENT -->');
if ($_mcPos !== false) {
...
if($articleId>0 && ($expArticleId || $expMode==='update' || $exportTextOnly)){
...
<li data-i18n="step4">Нажмите <strong>"СКАЧАТЬ"</strong> — статьи скачаются в папку <code><?=htmlspecialchars($currentSite)?>/blog/</code></li>
...
<h3 data-i18n="file_format">Формат файла</h3><p data-i18n="file_format_desc">Мета-данные в <code>&lt;meta name="..." content="..."&gt;</code> передают настройки статьи: slug, заголовок, язык, теги, дату публикации, статус доступа, категорию, planned, описание и системные параметры. Формат: краткое описание — между <code>&lt;!-- CONTENT SEPARATOR BELOW --&gt;</code> и <code>&lt;!-- MAIN CONTENT --&gt;</code>, основной текст — после <code>&lt;!-- MAIN CONTENT --&gt;</code></p>
...
<div class="footer"><strong>Boostore.pro</strong> — <span data-i18n="footer_text">Управление статьями блога</span> &nbsp;|&nbsp; <a href="https://boostore.pro/ru/docs/api-integration/#hotengine-CommerceAPI" target="_blank" data-i18n="footer_docs">Документация API</a> &nbsp;|&nbsp; <a href="index.php" style="color:#00d4ff;" data-i18n="back_home">← На главную</a></div>
</div>
<script>
// ===== i18n Translations =====
var i18n = {
  ru: {
    ...
    'file_format_desc':'Мета-данные в &lt;meta name=&quot;...&quot; content=&quot;...&quot;&gt; передают настройки статьи. Формат: краткое описание — между &lt;!-- CONTENT SEPARATOR BELOW --&gt; и &lt;!-- MAIN CONTENT --&gt;, основной текст — после &lt;!-- MAIN CONTENT --&gt;',
    ...
    'ref_lang_label':'🌐 Язык эталонной статьи','export_fields_label':'📋 Поля для экспорта','export_article_id_label':'Article ID','export_category_id_label':'ID категорії','export_category_name_label':'Имя категории',
  ...
  en: {
    ...
    'file_format_desc':'Meta data in &lt;meta name=&quot;...&quot; content=&quot;...&quot;&gt; carries article settings. Format: short description between &lt;!-- CONTENT SEPARATOR BELOW --&gt; and &lt;!-- MAIN CONTENT --&gt;, main content after &lt;!-- MAIN CONTENT --&gt;',
  ...
  ua: {
    ...
    'file_format_desc':'Мета-дані в &lt;meta name=&quot;...&quot; content=&quot;...&quot;&gt; передають налаштування статті. Формат: короткий опис — між &lt;!-- CONTENT SEPARATOR BELOW --&gt; та &lt;!-- MAIN CONTENT --&gt;, основний текст — після &lt;!-- MAIN CONTENT --&gt;',
    'actions_title':'⚡ Дії',
